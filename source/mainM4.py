...
from util import get_data, standardize_series, compute_class_weights
from networks import build_baseline, build_network
import argparse

BASE_PATH = 'SET_YOUR_BASE_PATH'
...
if __name__ == '__main__':
    ...
    if SCALE_LOSS:
        f_model.save('./saved_model_M4/' + SERIES_TYPE + '/main/f_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        w_model.save('./saved_model_M4/' + SERIES_TYPE + '/main/w_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        reg_model.save('./saved_model_M4/' + SERIES_TYPE + '/main/reg_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        cls_model.save('./saved_model_M4/' + SERIES_TYPE + '/main/cls_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        history_df = pd.DataFrame(history.history)
        history_df.to_csv('./saved_model_M4/' + SERIES_TYPE + '/main/history_' + str(MAX_LEN) + '_' + str(LAMBDA) + '.csv', index=False)
    else:
        f_model.save('./saved_model_M4/' + SERIES_TYPE + '/main_no_scale/f_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        w_model.save('./saved_model_M4/' + SERIES_TYPE + '/main_no_scale/w_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        reg_model.save('./saved_model_M4/' + SERIES_TYPE + '/main_no_scale/reg_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        cls_model.save('./saved_model_M4/' + SERIES_TYPE + '/main_no_scale/cls_model_' + str(MAX_LEN) + '_' + str(LAMBDA))
        history_df = pd.DataFrame(history.history)
        history_df.to_csv('./saved_model_M4/' + SERIES_TYPE + '/main_no_scale/history_' + str(MAX_LEN) + '_' + str(LAMBDA) + '.csv', index=False)
