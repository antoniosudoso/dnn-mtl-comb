...
df_errors <- readRDS(file=paste0(paste0("val/raw_labels_", SERIES_TYPE), ".rds"))
m <- compute_label_weights(df_errors)
saveRDS(m, file=paste0(paste0("val/train/qp_weights_", SERIES_TYPE), ".rds"))

# obtain hard labels from soft labels
tau <- 0.1 # adapt this threshold via cross-validation
m_labels <- m
m_labels[m_labels > tau] <- 1
m_labels[m_labels <= tau] <- 0
print(round(colSums(m_labels)/nrow(m_labels)*100, 2))
saveRDS(m_labels, file=paste0(paste0("val/train/qp_labels_", SERIES_TYPE), ".rds"))
