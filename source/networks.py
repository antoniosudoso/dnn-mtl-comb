...
def build_network(ts_length, f_methods, f_horizon, lambda_hyper, mu_hyper, scale_loss=True, class_weights=None):
    """
    :param ts_length: sequence length
    :param f_methods: number of forecasting methods
    :param f_horizon: forecasting horizon
    :param lambda_hyper: penalty parameter for multi-label classification
    :param mu_hyper: penalty parameter for orthogonality
    :return: models
    """
    x_raw = tf.keras.Input(shape=(ts_length, 1))
    ...
    if scale_loss:
        if class_weights is not None:
            full_model.compile(optimizer=opt,
                               loss=overall_loss_wrapper_class_weights(x_for, lambda_hyper, x_cls_true, x_cls_out, mu_hyper, x_reg_feat, x_cls_feat, class_weights),
                               metrics=[comb_loss_wrapper(x_for), cls_loss_wrapper_class_weights(x_cls_true, x_cls_out, class_weights), ort_loss_wrapper(x_reg_feat, x_cls_feat)])
        else:
            full_model.compile(optimizer=opt,
        ...
        if class_weights is not None:
            full_model.compile(optimizer=opt,
                               loss=overall_loss_wrapper_no_scale_class_weights(x_for, lambda_hyper, x_cls_true, x_cls_out, mu_hyper, x_reg_feat, x_cls_feat, class_weights),
                               metrics=[comb_loss_wrapper_no_scale(x_for), cls_loss_wrapper_class_weights(x_cls_true, x_cls_out, class_weights), ort_loss_wrapper(x_reg_feat, x_cls_feat)])
        else:
            full_model.compile(optimizer=opt,
                               loss=overall_loss_wrapper_no_scale(x_for, lambda_hyper, x_cls_true, x_cls_out, mu_hyper, x_reg_feat, x_cls_feat),
                               metrics=[comb_loss_wrapper_no_scale(x_for), cls_loss_wrapper(x_cls_true, x_cls_out), ort_loss_wrapper(x_reg_feat, x_cls_feat)])
