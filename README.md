...
Code to generate forecast combinations based on the paper ["Optimizing accuracy and diversity: a multi-task approach to forecast combinations"](https://doi.org/10.1007/s10479-026-07291-x).
...
The following scripts generate results for the M4 dataset. The user must specify the frequency (monthly, quarterly, yearly) using the variable `SERIES_TYPE`.
...
- **mainM4.py**: Trains the meta-learner on the M4 dataset. The resulting TensorFlow model is serialized in the `saved_model_M4` folder. 
- **mainLargeST.py**: Trains the meta-learner on the LargeST dataset. The resulting TensorFlow model is serialized in the `saved_model_LargeST` folder. 
