...
from util import get_data_LargeST, standardize_series
import argparse
import pandas as pd
...
if __name__ == '__main__':
    ...
    ts_train_list, ts_test_list, mat_forecast_list, div_label_list = get_data_LargeST(BASE_PATH, SERIES_TYPE, period='test')
    print('\tDone!')
    ...
    print('Saving true values...')
    df = pd.DataFrame(ts_test)
    df.to_csv('./saved_model_LargeST/' + SERIES_TYPE + '/t_' + SERIES_TYPE + '.csv', index=False)
    print('\tDone!')

    model_path = './saved_model_LargeST/' + SERIES_TYPE + '/main/f_model_' + str(LAMBDA)
    f_model = tf.keras.models.load_model(model_path, compile=False)
    f = f_model.predict([ts_train, ts_mat_forecast, ts_div_label])
    df = pd.DataFrame(f)
    df.to_csv('./saved_model_LargeST/' + SERIES_TYPE + '/f_' + SERIES_TYPE + '.csv', index=False)
    print('\tDone!')
